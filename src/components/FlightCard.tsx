...
function SegmentRow({
  ...
  const textSize = compact ? "text-sm" : "text-base sm:text-lg";

  return (
    <div className="flex items-center gap-2 sm:gap-3">
      {/* Departure */}
      <div className="text-left min-w-[56px] sm:min-w-[70px]">
        <div className={`${textSize} font-bold ${premium ? "text-white" : ""}`}>{first.departureTime}</div>
        <div className={`text-[10px] sm:text-[11px] leading-tight ${premium ? "text-gray-500" : "text-muted-foreground"}`}>
          {first.origin}
        </div>
      </div>

      {/* Timeline */}
      <div className="flex-1 flex flex-col items-center gap-0.5 px-0.5 sm:px-1 min-w-0">
        <div className={`text-[10px] sm:text-[11px] flex items-center gap-1 ${premium ? "text-gray-500" : "text-muted-foreground"}`}>
        ...
        <div className="w-full relative flex items-center h-4">
          ...
          <div className="absolute right-0 top-1/2 -translate-y-1/2">
            <Plane className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${premium ? "text-amber-400" : "text-primary"}`} />
          </div>
        </div>
      ...
      {/* Arrival */}
      <div className="text-right min-w-[56px] sm:min-w-[70px]">
        ...
        <CardContent className="p-0">
          ...
          <div className="flex flex-col sm:flex-row">
            ...
            <div className="flex-1 p-3 sm:p-4 md:p-5">
              ...
              <div className="flex items-center gap-2 sm:gap-2.5 mb-2.5 sm:mb-3">
                {proposal.airlineLogo && (
                  <img
                    ...
                    alt=""
                    className={`w-7 h-7 sm:w-8 sm:h-8 rounded-lg object-contain p-0.5 ${premium ? "bg-gray-800" : "bg-muted"}`}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className={`text-[10px] sm:text-[11px] ${premium ? "text-gray-500" : "text-muted-foreground"}`}>
                    {firstFlight?.flightNumber}
                    {firstFlight && ` \u00B7 ${firstFlight.departureDate}`}
                  </div>
                </div>
                {premium && (
                  <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-500/10 shrink-0">
              ...
              {/* Return */}
              {!isOneWay && inbound && (
                <div className={`mt-2 pt-2 sm:mt-2.5 sm:pt-2.5 border-t border-dashed ${premium ? "border-amber-500/10" : "border-border/40"}`}>
                  <SegmentRow flights={inbound} premium={premium} compact />
                </div>
              )}
            </div>

            {/* Price panel - horizontal bar on mobile, side column on desktop */}
            <div
              className={`flex items-center sm:items-end sm:flex-col justify-between sm:justify-between gap-3 sm:gap-0 p-3 sm:p-4 md:p-5 border-t sm:border-t-0 sm:w-52 md:w-56 shrink-0 ${
                premium
                  ? "sm:border-l sm:border-amber-500/10 bg-gray-950/40 border-t-amber-500/5"
                  : "sm:border-l sm:border-border/50 bg-muted/20 border-t-border/30"
              }`}
              ...
              {cheapest && (
                <>
                  ...
                  <div className="sm:text-right sm:w-full">
                    <div className={`text-2xl sm:text-3xl font-extrabold tracking-tight ${premium ? "text-amber-400" : "text-primary"}`}>
                      ${Math.round(cheapest.price)}
                    </div>
                    <div className={`text-[10px] sm:text-xs flex items-center sm:justify-end gap-1 mt-0.5 ${premium ? "text-gray-500" : "text-muted-foreground"}`}>
                      <Store className="h-3 w-3" />
                      {cheapest.gateName}
                    </div>
                    {/* Baggage - hidden on mobile, shown on desktop */}
                    <div className={`hidden sm:flex items-center justify-end gap-2 mt-1 text-[11px] ${premium ? "text-gray-500" : "text-muted-foreground"}`}>
                  ...
                  <div className="flex flex-col items-stretch shrink-0 sm:w-full">
                    ...
                    {/* Baggage info on mobile only */}
                    <div className={`flex sm:hidden items-center gap-1.5 mt-1.5 text-[10px] ${premium ? "text-gray-500" : "text-muted-foreground"}`}>
                      {cheapest.baggage.included ? (
                        <span className="inline-flex items-center gap-0.5 text-green-500">
          ...
          {/* More providers toggle + expanded gates */}
          {cheapest && proposal.gates.length > 1 && (
            <div
              className={`border-t px-3 sm:px-4 ${premium ? "border-amber-500/5" : "border-border/30"}`}
              onClick={(e) => e.stopPropagation()}
            >
              <button
                onClick={(e) => {
              ...
              >
                {showMoreGates ? "Hide providers" : `+${proposal.gates.length - 1} more provider${proposal.gates.length - 1 !== 1 ? "s" : ""}`}
          ...
          {/* Details hint bar */}
          <div className={`text-center py-1.5 text-[10px] sm:text-[11px] font-medium border-t transition-colors ${
            premium
            ...
            {proposal.gates.length > 1 && (
              <span className="sm:hidden">
                {` \u00B7 ${proposal.gates.length} providers`}
              </span>
            )}
          </div>
        </CardContent>
